...
class PrepareTab(QWidget):
    def __init__(self, main_window, wire_colors=None, spool_colors=None):
        ...
        # JSON Database pad bepalen (verwijst naar dezelfde materials.json als de pop-up)
        ...
        self.wire_colors = wire_colors if wire_colors else {"Koper": (0.8, 0.4, 0.2), "Zilver": (0.8, 0.8, 0.8), "Rood": (0.9, 0.1, 0.1)}
        self.spool_colors = spool_colors if spool_colors else {"Zwart Plastic": (0.1, 0.1, 0.1), "Wit Plastic": (0.9, 0.9, 0.9)}
    ...
    def init_ui(self):
        ...
        # AANPASSING 1: Dropdown breeder gemaakt (160px ipv 80px) zodat lange tekst erin past
        ...
        # Alleen de standaardscherm kleurendropdown vullen (niet de materiaaldropdown)
        for c_name, rgb in self.wire_colors.items():
            pix = QPixmap(14, 14); pix.fill(QColor.fromRgbF(*rgb))
            self.combo_wire_color.addItem(QIcon(pix), c_name)
            
        # Zorg dat de diameter mee-verandert als er een ander materiaal gekozen wordt
        self.combo_material.currentTextChanged.connect(self.on_material_dropdown_changed)

        # Rij 1: Materiaal Dropdown + Kladblok knop (lay-out aangepast voor de bredere dropdown)
    ...
    # AANPASSING 2: Haalt de schone lijst met namen op uit materials.json zonder kleur-iconen
    ...
    def open_materials_popup(self):
        ...
        # Nadat de pop-up sluit (.exec_() blokkeert de code tot sluiten), 
        # verversen we de dropdown direct zodat nieuwe/gewijzigde materialen zichtbaar worden!
        popup.exec()
    ...
    def retranslate_ui(self, tx):
        ...
        # Zorg dat de materiaallijst bij taalwissel herladen wordt en bewaar geselecteerde item
