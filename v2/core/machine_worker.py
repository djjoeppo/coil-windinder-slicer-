...
class MachineWorker(QObject):
    ...
    def emergency_stop(self):
        """Stopt direct met het verzenden van G-code en reset de status."""
        self._is_sending = False
        self._send_queue.clear()
        self._total_lines = 0  # <--- RESET NAAR 0 TOEGEVOEGD
        self._sent_lines = 0   # <--- RESET NAAR 0 TOEGEVOEGD
        ...
        if self.is_connected():
            try:
                ...
                self._ser.flush()
                self.line_received.emit("--- Programma gereset op machine ---")
            except Exception as e:
